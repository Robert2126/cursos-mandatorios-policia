...
El paquete se llama `api` y se ejecuta con el directorio raíz del repositorio como referencia (los módulos se importan como `api.main`, `api.config`, etc.), así que el entorno virtual se crea dentro de `api/` pero `uvicorn` se lanza desde la raíz.

```bash
cd api
cp .env.example .env
# Configure GEMINI_API_KEY y cambie todas las claves predeterminadas.
python -m venv .venv
...
`docker-compose.yml` construye la imagen usando la raíz del repositorio como contexto (necesario porque el paquete `api` se importa relativo a esa raíz).
